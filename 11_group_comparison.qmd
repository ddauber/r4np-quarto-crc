...
```{r}
#| label: tbl-comparing-two-groups-unpaired
#| echo: false
#| tbl-cap: "Comparing two unpaired groups"
...
data <- tibble(
  ...
  function_test = c("`t_test(var.equal = TRUE)`", "`t_test(var.equal = FALSE)`", "`wilcox_test(paired = FALSE)`"),
  effect_size = c("Cohen's d", "Cohen's d", "Wilcoxon R"),
  function_es = c("`cohens_d()`", "`cohens_d()`", "`wilcox_effsize()`")
)
...
data  |> 
  ...
  # Adjust padding to create visual spacing between columns without adding borders
  tab_options(
    data_row.padding = px(20),           # Increase padding to simulate column spacing
    column_labels.padding = px(20),      # Apply same padding to column labels
    table.border.top.width = px(1),
    table.border.bottom.width = px(1),
    table.font.size = px(12)  # Change 8 to desired size in pixels
  ) |> 
...
```{r}
#| label: tbl-comparing-two-groups-paired
#| echo: false
#| tbl-cap: "Comparing two paired groups"

# Create the data as a tibble
data <- tibble(
  data = c("P", "NP"),
  test = c("T-Test", "Wilcoxon Signed Rank Test"),
  function_test = c("`t_test(paired = TRUE)`", "`wilcox_test(paired = TRUE)`"),
  effect_size = c("Cohen's d", "Wilcoxon r"),
  function_es = c("`cohens_d()`", "`wilcox_effsize()`")
)
...
```{r}
#| label: tbl-comparing-multiple-groups-unpaired
#| echo: false
...
data <- tibble(
  ...
  function_test = c(
    "`anova_test` (equal variances only)\n`oneway.test(var.equal = TRUE/FALSE)`",
    "`kruskal_test()`"
  ),
  effect_size = c("Eta squared", "Epsilon squared (rank)"),
...
data  |> 
  ...
  # Adjust padding to create visual spacing between columns without adding borders
  tab_options(
    data_row.padding = px(30),           # Increase padding to simulate column spacing
    column_labels.padding = px(20),      # Apply same padding to column labels
    table.border.top.width = px(1),
    table.border.bottom.width = px(1),
    table.font.size = px(12)  # Change 8 to desired size in pixels
...
# Set specific widths for spacer columns to simulate horizontal space between main columns
# 140px widths works well for this book
  cols_width(
    data ~ px(35),
    test ~ px(65),
    function_test ~ px(150),
    effect_size ~ px(60),
    function_es ~ px(140)
    ) |>
  tab_footnote(
...
```{r}
#| label: tbl-post-hoc-tests
#| echo: false
#| tbl-cap: "Different post-hoc tests for different scenarios (parametric)"

# Create the dataset with underscores in variable names and no special characters
data <- tibble(
  equal_sample_size = c("YES", "NO*", "YES", "NO**"),
  equal_variances = c("YES", "YES", "YES", "NO"),
  post_hoc_tests = c(
    "- REGWQ,\n- Tukey,\n- Bonferroni",
    "- Gabriel",
    "- Hochberg's GT2",
...
data |> 
  ...
  fmt_markdown(columns = everything()) |>
  cols_label(
    equal_sample_size = "Equal sample size",
    equal_variances = "Equal variances",
    post_hoc_tests = "Post-hoc tests",
    functions_in_r = "Functions in R"
  ) |>
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_column_labels(everything())
  ...
  tab_options(
    data_row.padding = px(20),           # Increase padding for readability
    column_labels.padding = px(20),      # Adjust column label padding
    table.border.top.width = px(1),
    table.border.bottom.width = px(1),
    table.font.size = px(12)
  ) |>
  
# Table with 450px is good
  cols_width(
    equal_sample_size ~ px(55),
    equal_variances ~ px(50),
    post_hoc_tests ~ px(85),
    functions_in_r ~ px(260)
  ) |>
  tab_footnote(
    footnote = md("*Note: * slightly different only* | \\**\\* ideal for small samples*")
  )
```
...
```{r}
#| label: tbl-comparing-multiple-groups-paired
#| echo: false
#| tbl-cap: "Comparing multiple paired groups (effect size functions from package `effectsize`"

# Create the data as a tibble
data <- tibble(
  data = c("P", "NP"),
  test = c("Repeated measures ANOVA", "Friedman Test"),
  function_test = c("`anova_test()`", "`friedman_test()`"),
  effect_size = c("Eta squared", "Kendall's R"),
  function_es = c("`eta_squared()`", "`kendalls_w()`")
)
...
+-------------+--------------------------+----------------------------+
| Kendall's W | Interpretation           | Alternative interpretation |
|             |                          |                            |
...
```{r}
#| label: tbl-statistical-tests-unpaired-cat-variables
#| echo: false
#| tbl-cap: "Statistical tests to compare two unpaired categorical variables. Effect sizes are computed using the `effectsize` package"

# Create the data as a tibble
data <- tibble(
  matrix = c("2 x 2", "2 x 2", "n x n"),
  condition = c("< 10 obs.", "\\> 10 obs.", "\\> 5 obs.*"),
  test = c("Fisher's Exact Test", "Chi-squared Test with Yate's Continuity Correction", "Chi-squared Test"),
  function_test = c("`fisher.test()`", "`infer::chisq_test( correct = TRUE)`", "`infer::chisq_test()`"),
  effect_size = c("`phi()`", "`phi()`", "`cramers_v()`")
)
...
cols_width(
  ...
  tab_footnote(
    footnote = md("*Note: 80% of the cells*")
  )
```
...
# Effect size
effectsize::cohens_g(paired_data$pre_training,
                     paired_data$post_training)
```
...
## Exercises {#sec-exercises-group-comparisons}
...
```{r}
#| label: exercises-group_comparisons
#| eval: false
