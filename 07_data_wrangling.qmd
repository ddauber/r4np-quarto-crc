...
```{r}
#| label: drop-factor-levelsoption-02
#| echo: true
...
In @sec-descriptive-statistics, we will cover data visualisations like this boxplot in greater detail. If you cannot wait any longer and want to create your own visualisations, you are welcome to skip ahead. However, several data visualisations are awaiting you in the next chapter as well.
...
```{r}
#| label: fig-mnar-example
#| echo: false
#| fig-cap: "MNAR pattern in a dataset due to 'page-drop-offs'"
...
We now should have a dataset that is free of any missing values. To ensure this is the case we can create the missing data matrix that we made at the beginning of this chapter (see @fig-mapping-missing-data).
...
Now everything seems to make sense, and the internal consistency is excellent. We can assume that all three questions reflect one latent construct. From here, we can return to computing our latent variables (see @sec-computing-latent-variables) and continue with our analysis.
