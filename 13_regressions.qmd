# Regression: Creating models to predict future observations {#sec-regression}
...
# Set knitr chunk options and output width
...
A single linear regression looks very similar to a correlation (see @sec-correlations), but it is different in that it defines which variable affects another variable, i.e. a directed relationship. I used the terms *dependent variable (DV)* and *independent variable (IV)* previously when comparing groups (see @sec-comparing-groups), and we will use them here again. In group comparisons, the independent variable was usually a `factor`, but in regressions, we can use data that is not a categorical variable, i.e. `integer`, `double`, etc.
...
You might be wondering what $\beta_0$ stands for. It indicates an offset for each value, also called the intercept. Thus, no matter which value we choose for IV, DV will always be $\beta_0$ different from IV. It is a constant in our model. This can be best explained by visualising a regression line. Pay particular attention to the expressions after `\(x)`

```{r}
#| label: beta-zero-beta-one-explained
#| echo: true
#| fig-alt: "A plot with two diagonal lines that run in parallel. The blue line represents a beta zero of 1, while the red line represents a regression line with beta zero of 0."

# A: Two models with different beta(0)
ggplot() +
  geom_function(fun = \(x) 0 + x * 2, col = "red") +
  geom_function(fun = \(x) 1 + x * 2, col = "blue") +
  see::theme_modern()  
```

```{r}
#| label: beta-zero-beta-one-explained-diff
#| echo: true
#| fig-alt: "A plot with a red and blue regression line. Both lines have a shared origin at zero, but have different steepness."

# B: Two models with the same beta(0), but different beta(1)
ggplot() +
  geom_function(fun = \(x) 0 + x * 2, col = "red") +
  geom_function(fun = \(x) 0 + x * 3, col = "blue") +
  see::theme_modern()
```
...
### Fitting a regression model by hand, i.e. trial and error {#sec-fitting-a-regression-model-by-hand}
...
```{r}
#| label: fitting-model-by-hand-step-one
#| echo: true
#| fig-alt: "This scatter plot displays the relationship between ‘cumulative_cases’ and ‘cumulative_deaths’. Each point represents a data entry, showing that as cumulative cases increase, cumulative deaths tend to increase as well, indicating a positive association between these two variables. However, observations are clustered together in the bottom right-hand corner."
...
```{r}
#| label: fitting-model-by-hand-step-two
#| echo: true
#| warning: false
#| fig-alt: "This scatter plot illustrates the positive correlation between ‘cumulative_cases’ and ‘cumulative_deaths’. As cumulative cases increase, cumulative deaths tend to rise, with a strong upward trend suggesting a proportional relationship between these two variables."
...
```{r}
#| label: fitting-model-by hand-step-three
#| echo: true
#| warning: false
#| fig-alt: "This scatter plot shows the relationship between ‘cumulative_cases’ and ‘cumulative_deaths’. The points suggest a general upward trend, indicating that as cumulative cases increase, cumulative deaths also tend to rise, although with some variability among the data points."
...
```{r}
#| label: fitting-model-by-hand-step-four
#| echo: true
#| warning: false
#| fig-alt: "This scatter plot displays ‘cumulative_cases’ on the x-axis and ‘cumulative_deaths’ on the y-axis, with a red regression line indicating the trend between the two variables. The line suggests a positive association, but all observations fall below this line indicating a poor fit."

plot +
  geom_function(fun = \(x) x, col = "red")
```
...
```{r}
#| label: fitting-model-by-hand-step-five
#| echo: true
#| warning: false
#| fig-alt: "This scatter plot displays ‘cumulative_cases’ on the x-axis and ‘cumulative_deaths’ on the y-axis, with a red regression line indicating the trend between the two variables. The line suggests a positive association, and this time all observations fall roughly on this line indicating a better fit."

plot +
  geom_function(fun = \(x) 0.02 * x, col = "red")
```
...
```{r}
#| label: fitting-model-by-hand-step-six
#| echo: true
#| warning: false
#| fig-alt: "This scatter plot is similar to the previous one, but with minor adjustments to fit the red line onto the black dots even better."

plot +
  geom_function(fun = \(x) -50 + 0.015 * x, col = "red")
```
...
### Fitting a regression model computationally {#sec-fitting-a-regression-model-computationally}
...
```{r}
#| label: comparing-computed-vs-hand-fitted
#| echo: true
#| fig-alt: "This scatter plot shows ‘cumulative_cases’ on the x-axis and ‘cumulative_deaths’ on the y-axis, with two competing regression lines. The red line represents a manually fitted regression, while the blue line represents a computed regression line based on the data points. The slight divergence between the two lines suggests differences in accuracy. The red line is a less optimal fit."

plot +
  geom_function(fun = \(x) -50 + 0.015 * x, col = "red") +
    geom_function(fun = \(x) 88.1 + 0.01 * x, col = "blue")
```

With our final model computed, we also need to check its quality in terms of predictive power based on how well it can explain our observed data. We tested models before when we looked at confirmatory factor analyses for latent variables (see @sec-latent-constructs). This time we want to know how accurate our model is in explaining observed data and, therefore, how accurate it predicts future observations. The package `performance` offers a nice shortcut to compute many different indicators at once:

-   `check_model()`: Checks for linearity, homogeneity, collinearity and outliers

-   `model_performance()`: Tests the quality of our model.

For now, we are mainly interested in the performance of our model. So, we can obtain this information the following way:

```{r}
#| label: single-regression-model-performance
#| echo: true
...
Many of these indices will become more relevant when we compare models. However, $R^2$ can also be meaningfully interpreted without a reference model. We know that the bigger $R^2$, the better. In our case, it is `0.548`, which is very good considering that our model consists of only one predictor. It is not easy to interpret whether a particular $R^2$ value is good or bad. In our simple single linear regression, $R^2$ is literally *'r squared'*, which we already know from correlations and their effect sizes (see @tbl-effect-size-cohen). Thus, if we take the square root of $R^2$ we can retrieve the correlation coefficient, i.e. $r = \sqrt{R^2} = \sqrt{0.548} = 0.740$. According to @cohen1988statistical, this would count as a large effect size.
...
```{r}
#| label: predict-values-linear-regression
#| echo: true

# A dataset with values for cumulative cases of COVID
df_predict <-
...
```{r}
#| label: mr-computation-new-cases
#| echo: true
...
```{r}
#| label: mr-computation-new-cases-performance
#| echo: true
...
#| label: final-regression-model-no-masks-gatherings
#| echo: true
...
```{r}
#| label: finale-regression-model-no-masks-gatherings-performance
#| echo: true
...
+------------------------+--------------------------------------+---------------------------------------------------------------------------------------+
| *Independent variable* | -   Mahalanobis distance             | -   `mahalanobis()`[^13_regressions-3] or `mahalanobis_distance()`[^13_regressions-4] |
+------------------------+--------------------------------------+---------------------------------------------------------------------------------------+
...
: Outlier detection in multiple regressions {#tbl-outliers-in-multiple-regressions}

[^13_regressions-3]: This function does not take the model as an attribute, but instead requires the data, the column means and the covariance. Thus, we have to specify this function in the following way: `mahalanobis(data, colMeans(data), cov(data))`

[^13_regressions-4]: Function from `rstatix` package which automatically classifies values as outliers for us.
...
#### Outliers in the dependent variable {#sec-outliers-in-the-dependent-variable}
...
```{r}
#| label: externally-studentised-residuals-computation
#| echo: true
...
```{r}
#| label: externally-studentised-residuals-added
#| echo: true
...
```{r}
#| label: externally-studentised-residuals-nr-of-outliers-th-two
#| echo: true
...
```{r}
#| label: externally-studentised-residuals-nr-of-outliers-th-three
#| echo: true
out_detect |> count(pm_3)
```

```{r}
#| label: externally-studentised-residuals-nr-of-outliers-th-three-point-five
#| echo: true
out_detect |> count(pm_35)
```

```{r}
#| label: externally-studentised-residuals-nr-of-outliers-th-four
#| echo: true
out_detect |> count(pm_4)
```

The results indicate we could have as many as `56` outliers and as little as `9`. It becomes apparent that choosing the right threshold is a tricky undertaking. Let's plot the data again to make it easier to read and add some of the thresholds. I skip `3.5` since it is very close to `3`. I also reorder the observations (i.e. the x-axis) based on `.studresid` using `reorder()` which improves readability substantially.

```{r}
#| label: plot-outliers-ext-stud-resid
#| echo: true
#| fig-alt: "This plot shows ordered standardized residuals (studresid) with reference lines in green, orange and red, marking significant thresholds. Points outside these lines highlight potential outliers."
...
There are still more diagnostic steps we have to take before deciding which observations we want to remove or deal with in other ways (see also @sec-dealing-with-outliers).

#### Outliers in independent variables {#sec-outliers-in-the-independent-variables}
...
#| label: avg-lvg-plot
#| echo: true
#| fig-alt: "This plot shows ordered leverage values (.hat) for observations, with n orange horizontal line marking a threshold for high leverage, indicating potential outliers."
...
```{r}
#| label: mahalanobis-distance-computation
#| echo: true

mhnbs_outliers <-
  m1_outliers |>
  
  # Select only columns which represent independent variables
  select(new_cases:travel) |>
  
  # Compute the Mahalanobis distance
  rstatix::mahalanobis_distance() |>
  
  # Reformat the output to fit our 'outliers' dataset
  rownames_to_column() |>
...
```{r}
#| label: chi-square-critical-value
#| echo: true
...
The `p`-value reflects the probability we are willing to accept that our result is significant/not significant (remember Type I error in @sec-power-analysis). The value `df` refers to the degrees of freedom, which relates to the number of independent variables, i.e. `4`. Thus, it is fairly simple to identify a cut-off point yourself by choosing the `p`-value you consider most appropriate. The function `mahalanobis_distance()` assumes $p = 0.01$, which is certainly a good but strict choice.

If we want to plot outliers as before, we can reuse our code from above and replace it with the relevant new variables.

```{r}
#| label: mahalanobis-distance-plot
#| echo: true
#| fig-alt: "This plot displays ordered Mahalanobis distances, with an orange horizontal line indicating a threshold, used to identify potential outliers in the dataset."
...
#| label: cooks-d-plot
#| echo: true
#| fig-alt: "This plot shows Cook’s distance values across observations as a bar plot, highlighting points with high influence on the model fit. Peaks indicate potential influential data points of which there are at least three very obvious ones, but none reaches a value of one or higher."
...
```{r}
#| label: critical-value-f-distribution
#| echo: true
...
In the last chapter about outliers (@sec-reviewing-the-outliers), we will rerun the regression without outliers to see how this affects our model estimates. However, before we can do this, we have to cover one more method of detecting outliers.

#### Outlier detection considering the entire model: Specific measures {#sec-outlier-detection-specific-measures}
...
```{r}
#| label: dfbetas-plot-outliers
#| echo: true
#| fig-alt: "This plot visualizes the ‘movements’ variable across observations, with red lines marking threshold boundaries. Observations outside these boundaries may be considered outliers."
...
```{r}
#| label: dfbetas-plot-outliers-alternative
#| echo: true
#| fig-alt: "This plot shows the absolute values of ‘movements’ and ‘travel’ data points sorted in ascending order. A red line indicates a threshold level.Values above this line can be considered outliers related to COVID measures."

m1_dfbetas |>
  ggplot() +
  
  # Create dot plot for the variable movements
  ...
  # Create dot plot for the variable travel
  ...
  # Apply different colours to each dot plot
...
```{r}
#| label: example-of-pivoting-tibble
#| echo: true
...
```{r}
#| label: data-after-pivoting-longer
#| echo: true
...
-   We excluded `rownames` from being pivoted, i.e. `cols = !rowname`. The `!` means *not* in this context.
...
```{r}
#| label: count-outliers-after-pivoting
#| echo: true
...
```{r}
#| label: number-of-outliers-plot
#| echo: true
#| fg-alt: "A bar plot which displays how many analytical tools identified the same observation as an outlier. Most rows show only a score of 1, with fewer rows at values of 2 and 3."
...
As highlighted in @sec-dealing-with-outliers, there are many ways we can go about outliers. To keep our analysis simple, I intend to remove those observations detected by multiple methods rather than only by one. Whether this approach is genuinely appropriate is a matter of further investigation. I take a very pragmatic approach with our sample data. However, it is worth reviewing the options available to transform data, as covered by @field2013discovering (p.203). Data transformation can help to deal with outliers instead of removing or imputing them. Still, data transformation comes with severe drawbacks and in most cases, using a bootstrapping technique to account for violations of parametric conditions is often more advisable. Bootstrapping refers to the process of randomly resampling data from your dataset and rerun the same test multiple times, e.g. 2000 times. Thus, there will be some samples that do not include the outliers. The process is somewhat similar to multiple imputation (see @sec-replacing-removing-missing-data). Still, instead of estimating a specific value in our dataset, we estimate statistical parameters, e.g. confidence intervals or regression coefficients (i.e. estimates). The package `rsample` allows implementing bootstrapping straightforwardly. However, bootstrapping comes at the cost of lower power because we are running the test so many times.
...
m1_no_outliers <- anti_join(m1_outliers, outliers_select,
                            by = "rowname")
...
```{r}
#| label: mr-before-after-outlier-removal
#| echo: true
#| warning: false
...
parameters::compare_parameters(m1, m2) |>
  ...
  mutate(across(where(is.numeric), \(x) round(0, 3)))
...
### Standardised beta ($\beta$) vs. unstandardised beta ($B$) {#sec-standardised-beta-vs-unstandardised-beta}

In multiple regressions, we often use variables that are measured in different ways and which use different measurement units, e.g. currency, age, etc. Thus, it is sometimes fairly difficult to interpret regression coefficients without standardising the measurement units. Standardised scores (also called *z-scores*) imply that each variable has a distribution where the mean equals `0` and the standard deviation equals `1`. By transforming our variables to z-scores, they become 'unit free' [@cohen2014applied] (p. 25) and are easier to interpret. Here is an example with a simplified dataset:

```{r}
#| label: standardisation-variable-z-scores
#| echo: true
...
```{r}
#| label: scaled-unscaled-data-comparison
#| echo: true
#| fig-alt: "This line plot shows the variable 'x' plotted against its scaled version 'x_scaled', displaying a linear relationship. Each point on the line represents an increase in 'x' with a corresponding increase in 'x_scaled', indicating consistent scaling."
...
```{r}
#| label: correlation-scaled-unscaled-variables
#| echo: true
...
```{r}
#| label: test-regression-model-std-betas
#| echo: true
...
Considering the outcome, it would seem that both variables are good predictors for `salary` and their `Estimate`s are almost the same. Thus, one could be fooled into thinking that they are equally important. We can repeat the analysis but obtain unstandardised $\beta$s.

```{r}
#| label: test-regression-model-nonstd-betas
#| echo: true
...
This time the `Estimate`s are quite different from each other, i.e. $work\_experience = 2.42$ and $mindfulness = 0.50$. Considering these results it becomes clear that `work_experience` has a much stronger impact on `salary` than `mindfulness`. We can further confirm this by graphing the regression lines for each variable separately.

```{r}
#| label: visualising-swm-regression
#| echo: true
#| fig-alt: "This scatter plot shows the relationship between 'mindfulness' (blue points), 'work_experience' (red points) with regards to 'salary'. Two trend lines indicate distinct patterns for each variable’s impact on salary."
...
```{r}
#| label: scaling-with-parameters-package
#| echo: true
...
```{r}
#| label: scaling-with-parameters-package-no-refitting-model
#| echo: true
...
### Multicollinearity: The dilemma of highly correlated independent variables {#sec-multicollinearity}
...
## Hierarchical regression {#sec-hierarchical-regression}
...
### Regressions with control variables {#sec-regressions-with-control-variables}
...
```{r}
#| label: contrast-categorical-variables
#| echo: true
...
```{r}
#| label: contrasts-categorical-variables-more-levels
#| echo: true
...
```{r}
#| label: hr-m1-m2-performance-comparison
#| echo: true
...
```{r}
#| label: hr-compare-m1-m2-anova
#| echo: true

anova(m1, m2)
```

The results confirm that our model significantly improved. You might argue that this is not surprising because we added those variables which already worked in the final model of @sec-multicollinearity. However, the important takeaway is that our control variable `country` helps us explain more variance in `new_cases`. Comparing the model with and without the control variable, we would find that the $adjusted\ R^2$ improves our model by about 25%. As such, it is worth keeping it as part of our final regression model. Here is evidence of this improvement:

```{r}
#| label: hr-compare-m0-m2-performance
#| echo: true
...
### Moderated regression {#sec-moderated-regression}
...
```{r}
#| label: fig-plotting-moderation-effects-schools
#| echo: true
#| fig-cap: "Plotting moderation effects for `schools`."
#| fig-alt: "This scatter plot shows the relationship between the school measures taken (x-axis) and new COVID-19 cases (y-axis) in Germany and the United Kingdom. Germany’s data points and trend line are shown in red, while the United Kingdom’s are in teal. The trend lines indicate a stronger positive correlation between school measures taken and new cases in the United Kingdom compared to Germany."
...
```{r}
#| label: plotting-moderation-effects-businesses
#| echo: true
#| fig-cap: "Plotting moderation effects for `business`."
#| fig-alt: "This scatter plot shows the relationship between the amount of business measures taken (x-axis) and new COVID-19 cases (y-axis) in Germany and the United Kingdom. Germany’s data points and trend line are shown in red, while the United Kingdom’s are in teal. The trend lines indicate a stronger positive correlation between business measures taken and new cases in the United Kingdom compared to Germany."
...
```{r}
#| label: moderated-regression-m0-m4-performance
#| echo: true
...
```{r}
#| label: moderated-regression-m0-m4-anova
#| echo: true
...
```{r}
#| label: regression-for-centering
#| echo: true
...
```{r}
#| label: regression-for-centering-performance
#| echo: true
...
In other words, to achieve a centered variable ($x_{n_{centered}}$), we have to take the original observation ($x_n$) and subtract the mean of the variable ($\bar{x}$). How we define $\bar{x}$ determines whether we perform grand mean centering (using the entire dataset) or group mean centering (using subsets of our data, i.e. groups). We already know all the functions needed to compute centered variables. Since the `age` of `0` is not meaningful in our study, we should center this variable around its mean[^13_regressions-5].

[^13_regressions-5]: It is not required to choose the mean as a way to center variables. Any value that is meaningful can be used. However, the mean often tends to be the most meaningful value to perform centering.

```{r}
#| label: centering-by-mean-by-group
#| echo: true
...
```{r}
#| label: group=mean-centering-by-country
#| echo: true
...
```{r}
#| label: regression-centered-variables
#| echo: true
...
parameters::compare_parameters(model, model_c) |>
  as_tibble() |>
  select(Parameter, Coefficient.model, p.model,
         Coefficient.model_c, p.model_c) |>
  mutate(across(where(is.numeric), \(x) round(x, 3)))
```
...
```{r}
#| label: impact-of-centering-model-performance
#| echo: true
...
## Other regression models: Alternatives to OLS {#sec-ols-alternatives}
...
## Exercises {#sec-exercises-regressions}

After such a long chapter it is time to recap what we learned. Use the code provided to practice modeling and interpreting relationships. Remember, you have to install the `r4np` package first, if you have not done so yet.

```{r}
#| label: exercises-regressions
#| eval: false
